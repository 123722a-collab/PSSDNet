...
# ====== Face-Xray / bi_online_generation 可选依赖检测 ======
...
class SBI_Dataset(Dataset):
    def __init__(self, phase="train", image_size=224, n_frames=8, use_clip_norm=True):
        """
        phase: 'train'/'val'/'test'
        image_size: 输出图像尺寸（建议 CLIP 用 224）
        n_frames: 取帧数（init_ff 内部使用）
        use_clip_norm: 是否在 collate_fn 中做 CLIP mean/std 归一化（默认 True）
        """
        assert phase in ["train", "val", "test"]

        image_list, label_list = init_ff(phase, "frame", n_frames=n_frames)
        # image_list,label_list=init_cdf()

        path_lm = "/landmarks/"
        # 过滤：保证 landmarks 和 retina bbox 都存在
        label_list = [
            label_list[i]
        ...
        # ✅ 新增：是否输出 CLIP 标准化后的 pixel_values
        self.use_clip_norm = bool(use_clip_norm)

        # 训练集计数器（你原来写法里有缩进问题，这里整理成可运行版本）
        if phase == "train":
            if not hasattr(SBI_Dataset, "landmark_flip_count"):
                SBI_Dataset.landmark_flip_count = 0
            if not hasattr(SBI_Dataset, "total_train_samples"):
                SBI_Dataset.total_train_samples = 0
            ...
            try:
                ...
                if self.phase == "train":
                    if np.random.rand() < 0.5:
                        ...
                        # --- 增加计数器 ---
                        SBI_Dataset.landmark_flip_count += 1

                    # --- 增加总样本计数 ---
                    SBI_Dataset.total_train_samples += 1
                # -----------------------
                ...
                img_f = (
                    cv2.resize(img_f, self.image_size, interpolation=cv2.INTER_LINEAR)
                    .astype("float32")
                    / 255.0
                )
                img_r = (
                    cv2.resize(img_r, self.image_size, interpolation=cv2.INTER_LINEAR)
                    .astype("float32")
                    / 255.0
                )

                img_f = img_f.transpose((2, 0, 1))  # CHW
                img_r = img_r.transpose((2, 0, 1))  # CHW
                flag = False
    ...
    def self_blending(self, img, landmark):
        ...
        if exist_bi:
            ...
            try:
                ...
                print(
                    f"DEBUG: random_get_hull failed: {e}, falling back to convex hull"
                )
                mask = np.zeros_like(img[:, :, 0])
                cv2.fillConvexPoly(mask, cv2.convexHull(landmark), 1.0)
            logging.disable(logging.NOTSET)
        else:
            print("DEBUG: Not using Face-Xray, using simple convex hull")
            mask = np.zeros_like(img[:, :, 0])
            cv2.fillConvexPoly(mask, cv2.convexHull(landmark), 1.0)
    ...
    def collate_fn(self, batch):
        """
        返回：
          data['img']   : (2B,3,H,W) float32
          data['label'] : (2B,) long，前B=0(real) 后B=1(fake)

        ✅ 关键改动：如果 self.use_clip_norm=True，则对 data['img'] 做 CLIP mean/std 归一化，
        使其可直接喂给 CLIPVisionOnly / CLIPModel 的 pixel_values。
        """
        img_f, img_r = zip(*batch)

        # (B,3,H,W) in [0,1]
        real = torch.from_numpy(np.stack(img_r, axis=0)).float()
        fake = torch.from_numpy(np.stack(img_f, axis=0)).float()

        x = torch.cat([real, fake], dim=0)  # (2B,3,H,W)
        y = torch.tensor([0] * len(img_r) + [1] * len(img_f), dtype=torch.long)
...
if __name__ == "__main__":
    ...
    # ✅ 注意：如果你要喂 CLIP，建议 image_size=224
    image_dataset = SBI_Dataset(phase="test", image_size=256, use_clip_norm=True)
    ...
    # ⚠️ 你开启了 CLIP norm 后，img 不再是 [0,1]，直接 save_image 可能看起来很怪。
    # 如果你要可视化，建议临时 use_clip_norm=False，或反归一化后再保存。
